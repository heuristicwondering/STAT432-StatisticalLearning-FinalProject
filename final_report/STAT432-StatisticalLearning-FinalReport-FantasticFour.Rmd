...
As is evident in the plot, there does not seem to be a well defined elbow but beyond 4 clusters, there was an undeniable linear trend, so 4 was chosen by consenus. When examine the confusion matrix, it was clear that there was some limited success in accurately sorting connectivity matrics, but it largely preformed quite poorly.
...
```{r wcDistance, echo=FALSE, out.width="400px", fig.align="center", fig.cap="Amount of variance explained by top principle components"}
...
```{r PCelbow, echo=FALSE, out.width="400px", fig.align="center", fig.cap="Elbow plot of PC clustering"}
...
```{r PCclustering, echo=FALSE, out.width="400px", fig.align="center", fig.cap="Plots of Principle Component and Cluster Assignment"}
...
```{r PCplots, echo=FALSE, out.width="400px", fig.align="center", fig.cap="Plots of Principle Component Values"}
...
```{r PCconfusion, echo=FALSE, out.width="400px", fig.align="center", fig.cap="Confusion Matrix PC Components"}
...
The preceeding clustering approaches relied on local differences in neural coordination. However, it is well known that the brain exhibits global properties that are important for cognition (@bertolero2015modular). Therefore we attempted to cluster matrices on two widely used graph metrics in neuroscience, rich club structure 
<!--- What code snippets to put here? --->
...
Beyond k-means other algorithms were tested to see if they might be able to surpass the current best performance. Both Support Vector Machines applied to the correlation matrices pre-stimuli reassignment, and random forests post-reassignment showed results that seemed little better than random assignment. Thus they are not reported in full here, but mentioned as algorithms that do not seem appropriate for brain network clustering.
## SVM
## Random Forests
...
# Discussion
<!-- What are the big takeaways here? What worked? What failed especially well? What are the recommendations for researchers trying to choose a clustering algorithm? What are the limitations of this work? What are the future directions? -->
