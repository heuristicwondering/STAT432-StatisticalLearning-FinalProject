...
```{r connImages, echo=FALSE, fig.align="center", fig.cap="Voxelwise contrasts showing significant results"}
...
### Linear Regression
Linear regression was again used to assess for 
...
## @Megan about the fmri images and brain connectivity plots based on stimuli


## Description for the voxel intensity vs anatomical region of interest
For a preliminary analysis regarding the correlation between the voxel intensity and the stimulus, we averaged the intensities for each region over all the stimuli and plotted the average intensity vs. the anatomical brain region of interest. This stacked area graph represents all the five different stimuli, with ISI being the baseline. The other 4 stimili (EmoLabelPos, EmoLabelNeg, EmoLookPos, EmoLookNeg) are stacked on top of it. From this graph, we can visualize whether the brain activity in a partcular region changes w.r.t. the stimuli or if the activity remains constant irrespective of the stimuli. From this graph, we did observe abrupt changes in the mean voxel intensity for certain regions (depicted by the step like pattern towards the top right) w.r.t to the control ISI intensity. This tells us that the vexel intensities do capture the brain changes in intensity w.r.t the stimuli.


## t-test to find if the left and right anatomical hemispheres are actiavted equally 

One of the goals of this study was to see whether both the left and right hemispheres of the brain are involved to the same extent in responding towards a stimulus. Brain laterality refers to the fact that certain neural functions and cognitive processes are exclusively processed by one side of the brain. We wanted to see if the brain regions that we were looking for also followed this paradigm or if there was no laterality in the data. To test this assumption, we performed an paired, 2 -tail t-test for all the regions of a given stimulus between the left and right brain regions. Before performing the t-test, we tested for the distribution of the data to ensure that all the necessary conditions to perform the hypothesis test are met. 

## Here goes the density distribution graph.


The results that we got from the t-test were quite interesting. After accounting for the multiple testing correction (Benjamini Hochberg), it turned out that almost every single region of interest actually had a significantly different (p<0.05) mean voxel intensity between the left and the right regions. 

##More interpretation from Megan.


##Box plot from of the voxel intensity for each of the stimulus tested.

Another aspect of the data that we were interested in was to characterize the inter-region variability vs. the inter-stimulus variability of the voxel intensity. Based on the representative box-whisker plot (##attached below), it is evident that the inter-region variability is much higher compared to the inter-stimulus variability. This is because:---##Megan please add
